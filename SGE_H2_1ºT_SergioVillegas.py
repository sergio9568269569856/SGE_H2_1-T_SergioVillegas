...
def mostrar_grafico_barras():
    ...
    # Si prefieres un gráfico de líneas, usa este código
    # ax.plot(edades, bebidas, color='green', marker='o')  # Cambié a líneas con puntos
...
def exportar_a_excel(filtro):
    ...
    # Filtros según el criterio (puedes usar los mismos filtros que ya has usado)
    if filtro.get("id"):
        query += " AND idEncuesta = %s"
    ...
    df = pd.DataFrame(registros, columns=["idEncuesta", "edad", "Sexo", "BebidasSemana", "CervezasSemana", "BebidasFinSemana", "BebidasDestiladasSemana", "VinosSemana", "PerdidasControl", "DiversionDependenciaAlcohol", "ProblemasDigestivos", "TensionAlta", "DolorCabeza"])
...
def exportar_a_pdf(filtro):
    ...
    c.drawString(30, y_position, "ID Encuesta")
    c.drawString(150, y_position, "Edad")
    c.drawString(230, y_position, "Sexo")
    y_position -= 20

    # Datos
    for row in registros:
        c.drawString(30, y_position, str(row[0]))  # ID Encuesta
        c.drawString(150, y_position, str(row[1]))  # Edad
        c.drawString(230, y_position, str(row[2]))  # Sexo
        y_position -= 20
